...
from src.configs import ANNOTATION
...
class YOLOv7:
    def __init__(self, path, conf_thres=0.7, iou_thres=0.5):
        ...
        self.class_names = list(map(lambda x: x.strip(), open(ANNOTATION, 'r').readlines()))
        # Initialize model
    ...
    def draw_detections(self, image, boxes, scores, class_ids):
        for box, score, class_id in zip(boxes, scores, class_ids):
            ...
            labelSize, baseLine = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.2, 1)
            # top = max(y1, labelSize[1])
            # cv.rectangle(frame, (left, top - round(1.5 * labelSize[1])), (left + round(1.5 * labelSize[0]), top + baseLine), (255,255,255), cv.FILLED)
            cv2.putText(image, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), thickness=1)
        return image
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelpath', type=str, default='models/yolov7-tiny_256x320.onnx',
